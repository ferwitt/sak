...
class CallbackObject:
    def __init__(self, doc, root_cmd, args):
        ...
        self.run_button = pn.widgets.Button(name='Run', button_type='primary', width=250)
    ...
    def callback(self, **vargs):
        ...
        loading = pn.pane.GIF('https://upload.wikimedia.org/wikipedia/commons/b/b1/Loading_icon.gif', sizing_mode='stretch_both')
        #loading = pni.LoadingSpinner()
        self.doc.add_next_tick_callback(partial(self.update_doc, new_output=loading, stdout_str=None))
        ...
        try:
            ...
            def simple_update_stdout():
                ...
                while do_update_stdout:
                    ...
                    if self.stdout.object != stdout_str:
                        ...
                        self.doc.add_next_tick_callback(partial(self.update_stdout, stdout_str=stdout_str))
                    if do_update_stdout: time.sleep(UPDATE_PERIOD)
            ...
            self.doc.add_next_tick_callback(partial(self.update_doc, new_output=new_output, stdout_str=stdout_str + '\nDONE!'))
...
class SakDoc(param.Parameterized):
    ...
    def server_doc(self):
        ...
        tmpl = pn.template.BootstrapTemplate(title='SAK', header_background="#357ebd") #, theme=DarkTheme)
        ...
        controller = pn.Column(
            ...
            pn.pane.Markdown(subcmds_md, sizing_mode='stretch_width'), #, style={'font-size': '12px'}),
